...
from model.models import ActNet, img_show
import time
from jmdataload import  CreateDatasets, sort_dataset
from argparse import ArgumentParser
from omegaconf import DictConfig, OmegaConf
import torch.optim as optim
from losses import ComputeLosses, GradientDifferenceLoss
from sklearn.metrics import accuracy_score
from tqdm import tqdm
from misc import InitAverager, set_seed, split_list, save_images, save_checkpoint, VariableContainer, save_act, SegmentBuffer, TorchMetricsWrapper, MatrixSSIMPSNR, update_ssim_psnr, tens2act, repeat_data
import pandas as pd
import time, os
...
def main(xlstm_cfg: DictConfig):
    ...
    cd = CreateDatasets(xlstm_cfg=xlstm_cfg)
    ...
    file_path_list = sort_dataset(xlstm_cfg.datasets_path)
    valid_path_list, train_path_list = split_list(file_path_list, xlstm_cfg.valid_datas_scale)
    train_total_num = len(train_path_list)
    valid_total_num = len(valid_path_list)
    dataloader_train = cd.create_dataloader(train_path_list, phase='train', shuffle=True)
    dataloader_valid = cd.create_dataloader(valid_path_list, batch_size=1, phase='valid')
    ...
    # if True:
        for epc in range(xlstm_cfg.epochs):
            ...
            _ = train(model, optimizer, dataloader_train, device, compute_losses, writer, epc, train_total_num, train_avg, phase=train_phase) 
                 
            best_results = valid(model, optimizer, dataloader_valid, device, compute_losses, writer, epc, valid_total_num, valid_avg, phase=train_phase)
            ...
            if best_results.acts_dict is not None:
                ...
                new_actions_prediction = {**new_actions_prediction, **best_results.new_acts_dict}
            ...
            if not xlstm_cfg.olny_action_generate:
                ...
                metrix_side_ssim_frame = {
                    f'side_ssim_frame_{f_cnt+1}':f for f_cnt, f in enumerate(valid_avg.side_ssim_frames.avg)
                    }
                ...
                metrix_side_psnr_frame = {
                    f'side_psnr_frame_{f_cnt+1}':f for f_cnt, f in enumerate(valid_avg.side_psnr_frames.avg)
                    }
...
def train(model, optimizer, dataloader, device, compute_losses, writer, epc,   
          total_data, train_avg, phase=None):
    seg_buffer = SegmentBuffer(config=xlstm_cfg)
    timestamp = 0
    model.train()
    for batch_idx, (timestamp_motors_sucker, gripper_frame_timestamp, side_frame_timestamp, labels) in enumerate(dataloader):
        ...
        _time = time.time()
        output = model(act_segments, gripper_frame_timestamp, side_frame_timestamp, labels, phase)
        loss_results = compute_losses(output, labels, writer, epc, train_avg, phase)
        # compute acc
        pred_sucker = [torch.argmax(output.sucker[:, h, :], 1) for h in range(output.sucker.size(1))]
        ...
        print(f"\tPhase: training | Epc: {epc}/{xlstm_cfg.epochs} Total: {batch_idx}/{len(dataloader)} | losses: {loss_results.losses.item():.2f} | sucker_acc: {acc_sucker:.4f} | training time: {_run_time:.4f}", end='\r')
        break
    return loss_results

def valid(model, optimizer, dataloader, device, compute_losses, writer, epc, 
          total_data, valid_avg, seg_buffer=None, phase=None):
    model.eval()
    best_loss = 1e6
    timestamp = 0
    is_best_flag = None
    ...
    with torch.no_grad():
        for batch_idx, (timestamp_motors_sucker, gripper_frame_timestamp, side_frame_timestamp, labels) in enumerate(dataloader):
            ...
            timestamp_motors_sucker = repeat_data([timestamp_motors_sucker], xlstm_cfg.batchsize)[-1]
            gripper_frame_timestamp = repeat_data([gripper_frame_timestamp], xlstm_cfg.batchsize)[-1]
            side_frame_timestamp = repeat_data([side_frame_timestamp], xlstm_cfg.batchsize)[-1]
            gripper_frame_timestamp = gripper_frame_timestamp.transpose(2, -1)
            side_frame_timestamp = side_frame_timestamp.transpose(2, -1)
            labels[2] = labels[2].transpose(2, -1)
            labels[3] = labels[3].transpose(2, -1)
            ...
            # print(model)
            output, run_time = inference_time(model, act_segments, gripper_frame_timestamp, side_frame_timestamp, labels)
            ...
            if batch_idx in range((index) * full_data_len, (index + 1) * full_data_len):
                ...
                if (batch_idx + 1) % full_data_len == 0 and batch_idx > 0:
                    ...
                    calc_metrics(torch.cat(pred_new_act, dim=0), valid_avg, flag='new_act')
                    calc_metrics(torch.cat(pred_action, dim=0), valid_avg, flag='act')
                    calc_metrics(torch.cat(label_action, dim=0), valid_avg, flag='label')
                ...
                if not xlstm_cfg.olny_action_generate:
                    grip_results =  compute_ssimpsnr.compute(output.grip_future_seq, labels[2])  
                    side_results =  compute_ssimpsnr.compute(output.side_future_seq, labels[3])  
            ...
            print(f"\tPhase: valid | Epc: {epc}/{xlstm_cfg.epochs} | Total: {batch_idx}/{len(dataloader)} | losses: {loss_results.losses.item():.2f} | sucker_acc: {acc_sucker:.4f} | inference_time: {run_time:.4f}", end='\r')
