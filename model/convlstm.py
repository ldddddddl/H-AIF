...
from .model_utils import conv3d
...
class ConvLSTM(nn.Module):
    def __init__(self, input_channels, hidden_channels, in_b_n_h_w=(8, 3, 64, 32, 32), kernel_size=3, stride=1, padding=1, num_layers=1, first_flag=False, size_flag='remain', input_mode='stream'):
        ...
        for i in range(self.num_layers):
            if i == 0:
                if len(in_b_c_h_w) == 5 : 
                    ...
                    hidden_states.append((torch.zeros(input_tensor.shape[0], in_b_c_h_w[1], in_b_c_h_w[2], in_b_c_h_w[3], device=device),
                                        torch.zeros(input_tensor.shape[0], in_b_c_h_w[1], in_b_c_h_w[2], in_b_c_h_w[3], device=device)))
            else:
