...
class ActNet(nn.Module):
    def __init__(self, xlstm_cfg, is_use_cuda=False, device='vanilla'):
        ...
        self.act_norm3 = nn.BatchNorm1d((xlstm_cfg.max_history * xlstm_cfg.past_img_num)*xlstm_cfg.per_image_with_signal_num)
    ...
    def forward(self, timestamp_motors_sucker:torch.Tensor, gripper_frame_timestamp:torch.Tensor, side_frame_timestamp:torch.Tensor, labels:list, phase:str=''):
        ...
        if self.xlstm_cfg.visual_encoder == 'diffusion':
            ...
            grip_eq_hw = grip_conv_out_2_.transpose(1, 2).repeat(1, 1, 1, 2, 1)[:, :, :, :32, :]
            ...
            side_eq_hw = side_conv_out_2_.transpose(1, 2).repeat(1, 1, 1, 2, 1)[:, :, :, :32, :]
        ...
        for ith in range(self.xlstm_cfg.future_img_num + 1):
            ...
            if self.xlstm_cfg.is_use_mam:
                fused_modal = self.modal_fusion_model([grip_visual_out, side_visual_out, act_out])
            else:
                ...
                features = torch.cat([grip_out_temp, side_out_temp, act_out_temp], dim=-1)
            ...
            if ith < self.xlstm_cfg.future_img_num:
                if self.xlstm_cfg.is_diff_generate_act:
                    ...
                    self.output.act_diff = self.diffusion.sample(batch_size=self.xlstm_cfg.batchsize)
        ...
        if grip_future_seq != []:
            self.output.grip_future_seq = torch.cat(grip_future_seq[:self.xlstm_cfg.future_img_num], dim=1)
            self.output.side_future_seq = torch.cat(side_future_seq[:self.xlstm_cfg.future_img_num], dim=1)
